...
abstract class BaseCountry extends BaseObject  implements Persistent {
	...
	/**
	 * @var        array Suburb[] Collection to store aggregation of Suburb objects.
	 */
	protected $collSuburbs;

	/**
	 * @var        Criteria The criteria used to select the current contents of collSuburbs.
	 */
	private $lastSuburbCriteria = null;
	...
	public function setDisplayName($v)
	{
		...
		if ($this->display_name !== $v) {
			$this->display_name = $v;
			$this->modifiedColumns[] = CountryPeer::DISPLAY_NAME;
		}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collSuburbs = null;
			$this->lastSuburbCriteria = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collSuburbs !== null) {
				foreach ($this->collSuburbs as $referrerFK) {
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CountryPeer::DISPLAY_NAME)) $criteria->add(CountryPeer::DISPLAY_NAME, $this->display_name);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getSuburbs() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addSuburb($relObj->copy($deepCopy));
				}
			}
	...
	/**
	 * Clears out the collSuburbs collection (array).
	 *
	 ...
	 * @see        addSuburbs()
	 */
	public function clearSuburbs()
	{
		$this->collSuburbs = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collSuburbs collection (array).
	 *
	 * By default this just sets the collSuburbs collection to an empty array (like clearcollSuburbs());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initSuburbs()
	{
		$this->collSuburbs = array();
	}

	/**
	 * Gets an array of Suburb objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Country has previously been saved, it will retrieve
	 * related Suburbs from storage. If this Country is new, it will return
	 ...
	 * @throws     PropelException
	 */
	public function getSuburbs($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(CountryPeer::DATABASE_NAME);
		}
		...
		if ($this->collSuburbs === null) {
			if ($this->isNew()) {
			   $this->collSuburbs = array();
				...
				$criteria->add(SuburbPeer::COUNTRY_ID, $this->iso);

				SuburbPeer::addSelectColumns($criteria);
				$this->collSuburbs = SuburbPeer::doSelect($criteria, $con);
			}
		} else {
			...
			if (!$this->isNew()) {
				...
				SuburbPeer::addSelectColumns($criteria);
				if (!isset($this->lastSuburbCriteria) || !$this->lastSuburbCriteria->equals($criteria)) {
					$this->collSuburbs = SuburbPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastSuburbCriteria = $criteria;
		return $this->collSuburbs;
	}

	/**
	 * Returns the number of related Suburb objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related Suburb objects.
	 * @throws     PropelException
	 */
	public function countSuburbs(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(CountryPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collSuburbs === null) {
			if ($this->isNew()) {
				...
				$count = SuburbPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
	...
	/**
	 * Method called to associate a Suburb object to this object
	 * through the Suburb foreign key attribute.
	 *
	 * @param      Suburb $l Suburb
	 * @return     void
	 * @throws     PropelException
	 */
	public function addSuburb(Suburb $l)
	{
		if ($this->collSuburbs === null) {
			$this->initSuburbs();
		}
		if (!in_array($l, $this->collSuburbs, true)) { // only add it if the **same** object is not already associated
			array_push($this->collSuburbs, $l);
			$l->setCountry($this);
		}
	}
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			if ($this->collSuburbs) {
				foreach ((array) $this->collSuburbs as $o) {
